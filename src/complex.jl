...
function complex_cs_tssos_first(supp::Vector{Vector{Vector{Vector{UInt16}}}}, coe::Vector{Vector{T}}, n::Int, d; numeq=0, RemSig=false, nb=0, CS="MF", cliques=[], 
    ...
    if cliques != []
        ...
        time = @elapsed begin
        CS = CS == true ? "MF" : CS
        cliques,cql,cliquesize = clique_decomp(n, m, dc, supp, order=d, alg=CS; QUIET=QUIET)
        end
...
function clique_decomp(n, m, dc, supp::Vector{Vector{Vector{Vector{UInt16}}}}; order="min", alg="MF", QUIET=false)
    if alg == false
